...
FEATURE_HELP = {
    ...
    "IAA": "Percepção do aluno sobre o próprio desempenho no ciclo.",
}
FEATURE_DESCRIPTIONS = {
    ...
    "IAA": "Ajuda a entender a visão do aluno sobre si, mas pode superestimar a realidade acadêmica.",
}
ACTION_LIBRARY = {
    "IDA": (
    ...
    "IAA": (
        "Alinhar percepção e realidade",
        "Usar a autoavaliação como gatilho de conversa e calibrar expectativas com evidências objetivas do acompanhamento.",
    ),
}
NOTEBOOK_METRICS = {
    "Acurácia": "90,8%",
...
DEFAULT_BENCHMARK_ROW = {"media": 6.0, "mediana": 6.0, "q25": 5.0}
DEFAULT_IMPORTANCE_LOOKUP = {
    "IDA": 0.33,
    "IEG": 0.23,
    "IPV": 0.19,
    "IPS": 0.08,
...
MODEL_FEATURE_NAMES = FEATURES.copy()
...
def canonical_feature_name(feature: str) -> str:
    return str(feature).strip().upper()


def feature_label(feature: str) -> str:
    return FEATURE_LABELS.get(canonical_feature_name(feature), canonical_feature_name(feature))


def feature_help(feature: str) -> str:
    return FEATURE_HELP.get(canonical_feature_name(feature), canonical_feature_name(feature))


def feature_description(feature: str) -> str:
    return FEATURE_DESCRIPTIONS.get(
        canonical_feature_name(feature), "Indicador utilizado pelo modelo para apoiar a leitura do caso."
    )


def feature_action(feature: str) -> Tuple[str, str]:
    return ACTION_LIBRARY.get(
        canonical_feature_name(feature),
        (
            "Acompanhar indicador crítico",
            "Revisar esse sinal com a equipe e validar a melhor ação pedagógica ou socioemocional para o caso.",
        ),
    )
...
def estimator_display_name(model) -> str:
    names = {
    ...
    return names.get(model.__class__.__name__, model.__class__.__name__)
...
@st.cache_resource(show_spinner=False)
def infer_model_feature_names(_model) -> Tuple[List[str], List[str]]:
    artifact_features: List[str] = []
    if FEATURES_PATH.exists():
        try:
            loaded = joblib.load(FEATURES_PATH)
            artifact_features = [str(item) for item in loaded]
        except Exception:
            artifact_features = []

    raw_features = [str(item) for item in getattr(_model, "feature_names_in_", [])]
    if not raw_features:
        raw_features = artifact_features or FEATURES.copy()

    canonical_features = [canonical_feature_name(item) for item in raw_features]
    return raw_features, canonical_features
...
def load_analytics_base() -> Optional[pd.DataFrame]:
    ...
    for column in set(FEATURES + ACTIVE_FEATURES):
...
def get_feature_importances(model) -> pd.Series:
    try:
        if hasattr(model, "feature_importances_"):
            raw = pd.Series(model.feature_importances_, index=MODEL_FEATURE_NAMES, dtype="float64")
        elif hasattr(model, "coef_"):
            coefficients = model.coef_[0] if getattr(model.coef_, "ndim", 1) > 1 else model.coef_
            raw = pd.Series(coefficients, index=MODEL_FEATURE_NAMES, dtype="float64").abs()
        else:
            raise AttributeError("Modelo sem atributo de importância.")

        raw.index = [canonical_feature_name(index) for index in raw.index]
        grouped = raw.groupby(level=0).sum()
        for feature in ACTIVE_FEATURES:
            if feature not in grouped.index:
                grouped.loc[feature] = DEFAULT_IMPORTANCE_LOOKUP.get(feature, 0.05)
        grouped = grouped.reindex(ACTIVE_FEATURES).fillna(0.0)
        ...
        fallback = pd.Series(
            {feature: DEFAULT_IMPORTANCE_LOOKUP.get(feature, 0.05) for feature in ACTIVE_FEATURES}, dtype="float64"
        )
        total = float(fallback.sum())
        if total > 0:
            fallback = fallback / total
        return fallback.sort_values(ascending=False)
...
def get_benchmarks(df: Optional[pd.DataFrame]) -> pd.DataFrame:
    if df is None:
        return pd.DataFrame.from_dict(
            {
                feature: DEFAULT_BENCHMARKS.loc[feature].to_dict()
                if feature in DEFAULT_BENCHMARKS.index
                else DEFAULT_BENCHMARK_ROW
                for feature in ACTIVE_FEATURES
            },
            orient="index",
        )
    ...
    for feature in ACTIVE_FEATURES:
        if feature in df.columns:
            ...
            if series.notna().any():
                ...
                continue
        rows[feature] = (
            DEFAULT_BENCHMARKS.loc[feature].to_dict()
            if feature in DEFAULT_BENCHMARKS.index
            else DEFAULT_BENCHMARK_ROW.copy()
        )
...
def get_year_summary(df: Optional[pd.DataFrame]) -> pd.DataFrame:
    ...
    columns = ["INDE"] + [feature for feature in set(FEATURES + ACTIVE_FEATURES) if feature in df.columns]
...
def get_correlations(df: Optional[pd.DataFrame]) -> pd.Series:
    if df is None:
        fallback = DEFAULT_CORRELATIONS.copy()
        for feature in ACTIVE_FEATURES:
            if feature not in fallback.index:
                fallback.loc[feature] = 0.25
        return fallback.reindex(ACTIVE_FEATURES).fillna(0.25)

    available = [feature for feature in ACTIVE_FEATURES if feature in df.columns]
    corr = df[["INDE"] + available].corr(numeric_only=True)
    series = corr["INDE"].drop(labels=["INDE"]).sort_values(ascending=False)
    fallback = DEFAULT_CORRELATIONS.reindex(ACTIVE_FEATURES).fillna(0.25)
    return series.reindex(ACTIVE_FEATURES).fillna(fallback)
...
def predict_risk(model, input_series: pd.Series) -> Tuple[int, float]:
    payload = pd.DataFrame(
        [[input_series[canonical_feature_name(feature)] for feature in MODEL_FEATURE_NAMES]],
...
def build_priority_table(
    ...
    for feature in ACTIVE_FEATURES:
        ...
        importance = float(importances.get(feature, DEFAULT_IMPORTANCE_LOOKUP.get(feature, 0.05)))
...
def build_scenarios(
    ...
    priority_features = [
        row["Feature"]
        for _, row in build_priority_table(
            input_series,
            benchmarks,
            pd.Series({feature: DEFAULT_IMPORTANCE_LOOKUP.get(feature, 0.05) for feature in ACTIVE_FEATURES}),
        ).iterrows()
        if row["Gap"] > 0
    ][:3]
...
def build_recommendations(priority_df: pd.DataFrame) -> List[Tuple[str, str]]:
    ...
    for _, row in relevant.iterrows():
        recommendations.append(feature_action(row["Feature"]))
...
def plot_profile_comparison(input_series: pd.Series, benchmarks: pd.DataFrame):
    labels = [feature_label(feature) for feature in ACTIVE_FEATURES]
    base = benchmarks.loc[ACTIVE_FEATURES, "mediana"].astype(float)
    current = input_series.loc[ACTIVE_FEATURES].astype(float)
    colors = ["#0f766e" if current[feature] >= base[feature] else "#e76f51" for feature in ACTIVE_FEATURES]
...
def plot_correlation_chart(correlations: pd.Series):
    ...
    ax.barh([feature_label(idx) for idx in ordered.index], ordered.values, color=colors)
    ax.set_xlim(0, 0.85)
...
def plot_importance_chart(importances: pd.Series):
    ...
    fig, ax = plt.subplots(figsize=(6.6, 4.3))
    ax.barh([feature_label(idx) for idx in ordered.index], ordered.values, color="#f08a5d")
...
MODEL_FEATURE_NAMES, ACTIVE_FEATURES = infer_model_feature_names(model)
...
model_name = estimator_display_name(model)
primary_pillars = join_labels(importances.head(min(3, len(importances))).index.tolist()) if not importances.empty else join_labels(ACTIVE_FEATURES[:3])
indicator_count = len(ACTIVE_FEATURES)
...
with st.sidebar:
    ...
    st.write("1. Ajuste os indicadores disponíveis para o aluno.")
...
with tab_triagem:
    ...
    with col_preview:
        ...
        st.markdown(
            ...
            <div class="soft-card">
                ...
                <p>
                    {below_median} de {indicator_count} indicadores estão abaixo da mediana da base.
    ...
    result_cols[1].metric("Abaixo da mediana", f"{below_median}/{indicator_count}")
...
with tab_modelo:
    ...
    model_cols[0].metric("Algoritmo", model_name)
    model_cols[1].metric("Variável-alvo", "INDE abaixo da mediana")
    model_cols[2].metric("Features", str(indicator_count))
    ...
    with governance_left:
        ...
        importances_table = importances.rename(index=lambda feature: feature_label(feature)).reset_index()
    ...
    with governance_right:
        ...
        dictionary_rows = [
            {
                "Indicador": feature,
                "Leitura de negócio": feature_label(feature),
                "Como interpretar": feature_description(feature),
            }
            for feature in ACTIVE_FEATURES
        ]
